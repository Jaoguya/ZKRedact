...
trust-aware control \cite{ref16}. These mechanisms strengthen
...
\cite{ref1,ref6,ref19}. Nevertheless, authorization, CH
...
For each $1\le k\le\bar v_i$, it verifies
\begin{equation}
\mathsf{MerkleVerify}
...
Starting from the authenticated $c_i^{(0)}$, it reconstructs
\begin{equation}
\widehat c_i^{(k)}
=
H\left(
...
% Spans both columns. At 0.32\columnwidth each panel printed about 1.1 in
% against a 3.5 in design size, so the axis labels were unreadable; across
% \textwidth the same three panels are roughly 2.3 in each.
\begin{figure*}[!t]
    \centering
    \includegraphics[width=0.32\textwidth]{Images/1.pdf}
    \hfill
    \includegraphics[width=0.32\textwidth]{Images/2.pdf}
    \hfill
    \includegraphics[width=0.32\textwidth]{Images/3.pdf}
...
This experiment evaluates audit verification cost against the redaction
history depth $\nu$ of the audited transaction, with each scheme measured
at ledger sizes $L=10^2$ and $L=10^3$ blocks. A curve's slope is therefore
its dependence on history depth, and the gap between a scheme's two curves
is its dependence on ledger size.

As shown in Fig.~\ref{fig:combined_3_panel}(c), the two ZK-Redact curves
are nearly coincident: audit cost is governed by the audited transaction's
own history and not by the size of the ledger containing it, which is the
property the PAI is designed to provide. Ref.~\cite{ref22} separates by
roughly an order of magnitude between the two ledger sizes and remains
orders of magnitude more expensive in absolute terms, since its audit
collects the current version of every block.

ZK-Redact's cost does grow with $\nu$, and above $\nu\approx8$ exceeds
Refs.~\cite{ref10,ref13} in absolute terms. This is the $\nu$ term of the
$O(\nu+\log|\mathcal{I}|)$ bound and it is not overhead: verifying a
history of length $\nu$ re-verifies $\nu$ zero-knowledge authorization
proofs, establishing that each redaction was permitted under its
registered policy without revealing the attributes that satisfied it.
Ref.~\cite{ref10} re-verifies committee signatures, which identify the
approvers, and Ref.~\cite{ref13} performs a challenge-response spot check
on ledger integrity; neither audits per-redaction authorization, so
neither pays for it. The comparison is therefore one of scaling behaviour
and of what an audit establishes, not of equivalent work.
