...
|---|---|---|
| **Channel `BatchTimeout` is 250 ms, not a second or two** | Ref[10] votes through the ordering service, and a round is two blocks, so the block interval IS this baseline's dominant Exp 1 cost — measured on c6i.8xlarge at 1000 ms: **0.534 authorizations/s at concurrency 1, 1.87 s per authorization**, putting Exp 1's fabric arm at ~10.4 h with its c=1 point alone at 5.2 h. Shortening the interval measures the same protocol on a faster-ordering channel; the alternative was predicting the unmeasured concurrency levels from the measured ones, which SKILL.md rule 1 forbids — and which would have assumed away the saturation point and crossover Exp 1 exists to find. Same lever pulled once before, 2000 -> 1000 ms | **Favours Ref[10] in absolute latency, neutral in the comparison.** Every system is measured on the same channel, and only Ref[10] touches it at all — the other three authorize off-chain. Its published figures were never comparable to ours regardless. Two derived sweeps rescale with it, `zkredact.proof_batch.wait_bound_ms` and `zkredact.redaction_batch.wait_bounds_ms`, because both are defined as fractions of a block interval rather than as fixed durations. **Every fabric figure taken at 1000 ms must be re-measured** |
