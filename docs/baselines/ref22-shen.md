...
| **Communication is NOT included, matching the paper.** Ref[22] runs in-process; `internal/schemes/ref22` opens no connection and touches no Fabric peer | The paper draws the same boundary: *"the impact of communication is not considered in the test of each functionality"* ([Ref[22].md:775](../../Reference/Ref%5B22%5D/Ref%5B22%5D.md#L775)). An earlier version of this row claimed the opposite — that network cost "is included for all four systems identically". **That claim was false**: only Ref[10]'s voting round crosses a network, and the accumulator, the double-trapdoor CH and `ValChain` never did. Corrected rather than implemented, because Ref[22] defines no networked authorization protocol to charge for — inventing one would fabricate a cost, which is the failure mode `SKILL.md` exists to prevent | **Neutral between schemes.** `CryptoTime` is off-chain for all four, so no scheme's cryptography carries transport. `LedgerTime` is in-process here, and real only for Ref[10] — see the shared measurement boundary below |
...
| **`Modify` no longer regenerates every block's membership witness.** Witnesses are dropped when the accumulator moves and recomputed on demand, one exponentiation each via the regulator's `phi(N)` (`Accumulator.TrapdoorMembershipWitness`) | **Not a deviation — Algorithm 5 never asks for it.** [Ref[22].md:441](../../Reference/Ref%5B22%5D/Ref%5B22%5D.md#L441) has Modify "give witnesses `w_{i'}` for both the validity of `A_{i'}` and the non-membership of all history versions of the modified block", and line 506 defines `w_{i'}` as that block's NI-PoE / NI-PoKE tuple — singular, scoped to the block being modified. Refreshing the other `n-1` blocks was OUR bookkeeping, added so `ValChain` would not fail for reasons unrelated to redaction, and it ran `AllMembershipWitnesses` (RootFactor, O(n log n) exponentiations at RSA-3072) inside the timed `CryptoTime` region. The NI-PoE/NI-PoKE tuple Algorithm 5 does specify is still built and still timed | **Removes a 763x inflation of Ref[22]'s Exp 2 cost.** Measured at 1,000 blocks: 13.66 s per redaction before, 0.0179 s after, with `crypto_share` 1.0000 in both — the remaining cost is all real cryptography, not bookkeeping moved off the clock. Succeeded/stale/failed counts unchanged (20/0/0). At the configured 100,000-block dataset the old path extrapolated to ~37 min per redaction, which made Exp 2 unrunnable. This is the SECOND instance of the pattern in the row below: charging the baseline for upkeep its algorithm does not perform inflates it against ZK-Redact, which is the opposite of the error `allow_published_numbers_in_tables: false` guards against. `TrapdoorMembershipWitness` requires `phi(N)` and is therefore regulator-only; `VerifyMembership` is untouched, so Exp 3's verification cost is unchanged |
